...
struct Line
{
	...
	friend istream& operator>>(istream& s, Line& v) { Vector<T> a, b; cin >> a >> b; v = Line(a, b); return s; }
...
int main()
{
	...
	if (!segcrseg(a, b, c, d)) { cout << "Empty"; return 0; }
	if (x.A * y.B == x.B * y.A)
	{
		...
		if (pinseg(d, a, b))
		{
			if (find(ans.begin(), ans.end(), d) == ans.end())
				ans.push_back(d);
		}
		if (ans.size() == 1)
		{
		...
		Vector<long long> s = ans[0], r = ans[1];
		if (!((s.x < r.x) || ((s.x == r.x) && (s.y < r.y)))) swap(s, r);
		cout << s << ' ' << r;
	}
	else
	{
		Line<double> xx, yy;
		xx = Line<double>(double(x.A), double(x.B), double(x.C));
		yy = Line<double>(double(y.A), double(y.B), double(y.C));
		cout << intersect(xx, yy);
	}
