...
int main()
{
	...
	Vector<double> a, b, c; cin >> a >> b >> c;
	Line<double> l1(b, b + (c - a) / ((a - b).len() + (c - b).len()) * (a - b).len() + (a - b));
	Line<double> l2(c, c + (b - a) / ((a - c).len() + (b - c).len()) * (a - c).len() + (a - c));
	Vector<double> o = intersect(l1, l2);
	cout << o << ' ' << abs(Line<double>(a, b).dist(o));
