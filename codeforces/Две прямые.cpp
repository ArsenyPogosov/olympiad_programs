...
int main()
{
	...
	Line<double> l(a, b, c), r(d, e, f);
	cout << -l.dir() << '\n' << -r.dir() << '\n';
	if (l.A * r.B == l.B * r.A)
	{
		double m = l.A, n = r.A;
		if (m == 0) { m = l.B; n = r.B; }
		cout << abs(double(l.C * n - r.C * m) / l.norm().len() / n);
	}
	else
	{
		Line<double> x, y;
		x = Line<double>(double(l.A), double(l.B), double(l.C));
		y = Line<double>(double(r.A), double(r.B), double(r.C));
		cout << intersect(x, y);
	}
