...
int main()
{
	...
	Vector<long long> p; cin >> p;
	Vector<double> pp(double(p.x), double(p.y));
	if ((p - o).sqlen() > r * r)
	{
		cout << 2 << '\n';
		double p2p3 = ((pp - oo).sqlen() - r * r) * r * r / (pp - oo).sqlen();
		double pop3 = r * r - p2p3;
		cout << oo + resize(pp - oo, sqrt(pop3)) << '\n';
		double p1p3 = (pp - oo).len() - sqrt(pop3);
		cout << p1p3 << ' ' << sqrt(p2p3) << '\n';
		cout << oo + resize(pp - oo, sqrt(pop3)) + resize((pp - oo).norm(),sqrt(p2p3)) << '\n';
		cout << oo + resize(pp - oo, sqrt(pop3)) - resize((pp - oo).norm(), sqrt(p2p3)) << '\n';
	}
	if ((p - o).sqlen() == r * r)
	{
